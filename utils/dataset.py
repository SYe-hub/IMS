...
class Duorou(Dataset):
    def __init__(self, root, is_train=True, transform=None, target_transform=None, loader=default_loader):
        self.data_dir = root
        self.transform = transform
        self.target_transform = target_transform
        self.loader = loader
        # split_txt = os.path.join(self.data_dir, 'train_split_V7.2.txt' if is_train else 'test_split_V7.2.txt')
        # split_txt = os.path.join(self.data_dir, 'train_split.txt' if is_train else 'test_split.txt')
        # split_txt = os.path.join(self.data_dir, 'train_split_cd_s1s2.txt' if is_train else 'test_split_cd.txt')
        split_txt = os.path.join(self.data_dir, 'train_split_cd_s1.txt' if is_train else 'test_split_cd.txt')
        #split_txt = os.path.join(self.data_dir, 'train_split_cd_s1.txt' if is_train else 'test_split_V7.2_CD.txt')
        with open(split_txt, 'r', encoding='UTF-8') as f:
            # self.img_label_list = [(item.split()[0], int(item.split()[-1]) - 1) for item in f.read().splitlines()]
            self.img_label_list = [(item.split()[0], int(item.split()[-1])) for item in f.read().splitlines()]
        print('===>load Duorou datasets {} {} images...'.format(self.__len__(), 'train' if is_train else 'test'))
...
class CUB(Dataset):
    def __init__(self, root, is_train=True, transform=None, target_transform=None, loader=default_loader):
        ...
        img_txt = os.path.join(self.data_dir, 'images.txt')
        image_class_labels_txt = os.path.join(self.data_dir, 'image_class_labels.txt')
        train_test_split = os.path.join(self.data_dir, 'train_test_split.txt')

        with open(img_txt, 'r', encoding='UTF-8') as f:
            img_list = [item.split()[1] for item in f.read().splitlines()]
        with open(image_class_labels_txt, 'r', encoding='UTF-8') as f:
            img_class_label_list = [int(item.split()[1]) - 1 for item in f.read().splitlines()]
        ...
        for item in train_test_list:
            idx, flag = item.split()
            if int(flag) == 1:
                idx_train_list.append(int(idx) - 1)
            else:
                idx_test_list.append(int(idx) - 1)

        if is_train:
            self.imgs_path = [img_list[i] for i in idx_train_list]
            self.labels = [img_class_label_list[i] for i in idx_train_list]
        else:
            self.imgs_path = [img_list[i] for i in idx_test_list]
            self.labels = [img_class_label_list[i] for i in idx_test_list]

        print('===>load CUB datasets {} {} images...'.format(self.__len__(), 'train' if is_train else 'test'))
...
class Flower(Dataset):
    def __init__(self, root, is_train=True, transform=None, target_transform=None, loader=default_loader):
        ...
        # split_txt = os.path.join(self.data_dir, 'train_shift_2.txt' if is_train else 'test_shift_2.txt')
        # with open(split_txt, 'r', encoding='UTF-8') as f:
        #     self.img_label_list = [(item.split('/')[-1], int(item.split('/')[0]) - 1) for item in f.read().splitlines()]

        split_txt = os.path.join(self.data_dir, 'train.txt' if is_train else 'test.txt')
        with open(split_txt, 'r', encoding='UTF-8') as f:
            self.img_label_list = [(item.split()[0], int(item.split()[-1]) - 1) for item in f.read().splitlines()]
        print('===>load Flower datasets {} {} images...'.format(self.__len__(), 'train' if is_train else 'test'))
    ...
    def __getitem__(self, index):
        img_name, label = self.img_label_list[index]
        # img_path = os.path.join(self.data_dir, 'jpg', img_name)
...
class AirCraft(Dataset):
    ...
    def __init__(self, root='data/aircraft', transform=None,
        ...
        self.label = torch.tensor([self.imgs.iloc[i]['label'] for i in range(len(self.imgs))])
